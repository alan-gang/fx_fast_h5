...
import {
  getMethodsConfigByType,
  getMethodPosByGameTypeAndId
} from '../../game/gameMethods'
...
class Game extends Component<Props, object> {
  ...
  constructor(props: Props) {
    ...
    let curMenuIndex =
      bestLudan &&
      getMethodPosByGameTypeAndId(this.gameType, bestLudan.methodId)
    curMenuIndex = curMenuIndex === -1 ? 0 : curMenuIndex
    let curMenuEname = menus && menus[curMenuIndex] && menus[curMenuIndex].ename
    let ludanTab = getLudanTabByTypeAndName(
      this.gameType,
      curMenuEname,
      bestLudan && bestLudan.codeStyle
    )
    let ludanMenus = getTabsByType(this.gameType, curMenuEname)
    let gameLimitLevel = this.props.store.game.getGameLimitLevelByGameId(
      this.id
    ) // 设置的限红数据
    ...
    this.state = {
      ...
      curGameMethodItems: this.getMethodItemsByIds(
        (menus && menus[curMenuIndex] && menus[curMenuIndex].ids) || []
      ),
      odds: {},
      issueList: [],
    ...
    this.mysocket = new Socket(
      {
        ...
        receive: data => {
          if (data.type === 'openWinCode') {
            this.openWinCode(
              parseInt(data.content[0].lottId, 10),
              data.content[0]
            )
          }
  ...
  onSetLimit = () => {
    this.setState({
      isShowLimitSetDialog: !this.state.isShowLimitSetDialog,
      isShowLimitSetDialogClose: true,
      limitLevelList:
        this.props.store.game.getLimitDataOfKqByGameId(this.id) || []
    })
  }
  componentWillReceiveProps(nextProps: Props) {
    ...
    if (this.props.match.params.id !== nextProps.match.params.id) {
      ...
      this.setState({
        ...
        // isShowLudan: ludanMenus && ludanMenus.length > 0,
        isShowLimitSetDialog: !gameLimitLevel,
        limitLevelList: !gameLimitLevel
          ? limitListItem
            ? limitListItem.kqPrizeLimit
            : []
          : []
      })
      this.init()
  ...
  updateSubMethods = (method: GameSubMethodMenu) => {
    ...
    if (this.gameType === LOTTERY_TYPES.HC6) {
      ...
      if (this.state.curMenuEname === 'texiao') {
        curGameMethodItems = this.updateHC6TexiaoMethodItems(
          curGameMethodItems,
          method.v
        )
      }
  ...
  calcBet() {
    ...
    curGameMethodItems.forEach((gameMethodItem: any) => {
      method = {
        id: gameMethodItem.id,
        methodTypeName: gameMethodItem.methodTypeName,
        rows: [],
        repeatCount: 0
      }
    ...
    curGameMethodItems.forEach((gameMethodItem: any) => {
      if (['zx_q2', 'zx_q3'].includes(gameMethodItem.methodTypeName)) {
        gameMethodItem.repeatCount = countRepeat(
          methodList.map((methodItem: DataMethodItem) =>
            methodItem.id === gameMethodItem.id ? methodItem.rows : []
          )
        )
      }
    })
    ...
    methodList.forEach((methodItem: DataMethodItem) => {
      betCount += this.calc[methodItem.id]({
        nsl: methodItem.rows,
        ns: methodItem.rows,
        repeatCount: methodItem.repeatCount
      })
      // 任选，组选，直选金额计算
      if (
        ['rx_nzn', 'zux_q2', 'zux_q3', 'zx_q2', 'zx_q3'].includes(
    ...
    APIs.curIssue({ gameid }).then((data: any) => {
      if (data.success > 0) {
        this.setState({
          ...
          remainTime:
            Math.floor((data.saleend - data.current) / 1000) ||
            this.state.remainTime + 0.05
        })
      } else {
  ...
  getLimitData(id: number) {
    APIs.lottSets({ lotteryIds: id, v: 1 }).then((data: any) => {
      if (data.success === 1) {
        this.props.store.game.setLimitList([
          Object.assign({ id }, data.data[id])
        ])
  ...
  updateHC6TexiaoMethodItems(
    methodItems: any,
    subMethodMenuValue: string = ''
  ) {
  ...
  updateInfoWithBestLudan(bestLudan: BestLudanItem) {
    ...
    this.setState({
      ...
      subMethods:
        (menus && menus[curMenuIndex] && menus[curMenuIndex].subMethods) || [],
      defaultMenu: (ludanTab && ludanTab.name) || '',
      defaultSubMenu:
        ludanTab && ludanTab.subM && ludanTab.subM.length > 0
          ? bestLudan.codeStyle.split('_')[1]
          : '',
      isShowLudan: ludanMenus && ludanMenus.length > 0
    })
  }
    ...
    return (
      <article className="game-view">
        <GameCommonDataContext.Provider
          value={{ gameId: this.id, gameType: this.gameType }}
        >
          <GameHeader
            gameId={this.id}
          ...
          <section className="game-main">
            <MethodMenu
              ...
              methodMenuChangedCB={this.methodMenuChangedCB}
              updateMethodMenuIndex={this.updateMethodMenuIndex}
            ...
            <Play
              ...
              gameType={this.gameType}
              curMenuIndex={this.state.curMenuIndex}
              defaultInitMethodItemAmount={
                this.state.defaultInitMethodItemAmount
              }
            ...
            <OrderBar
              ...
              updateDefaultInitMethodItemAmount={
                this.updateDefaultInitMethodItemAmount
              }
